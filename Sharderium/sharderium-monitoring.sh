...
while true; do
    ...
    CONTAINER_STATUS=$(docker inspect -f '{{.State.Running}}' shardeum-validator)
    if [[ "$CONTAINER_STATUS" != "true" ]]; then
        send_telegram_message "Контейнер shardeum-validator не запущений. Спроба запуску..."
        docker start shardeum-validator
        sleep 10
    fi

    # Отримуємо саме поле state
    STATE=$(docker exec -t shardeum-validator operator-cli status | awk '/state:/ {print $NF}' | tr -d '[:space:]')
